...
def calculate_speeds(points):
    """
    Calculate speeds between consecutive points based on distance and time differences.
    Also provides estimated speeds even for points without time data.
    
    :param points: list of dict with lat, lon, time
    :return: list of same points with 'speed' added (km/h)
    """
    if len(points) < 2:
        return points
        
    # Ensure first point has speed=0
    points[0]['speed'] = 0
    
    # First pass: Calculate speeds for points with time data
    points_with_speed = []
    total_distance = 0
    total_time = 0
    ...
    for i in range(1, len(points)):
        ...
        # Calculate distance regardless of time availability
        dist = haversine_distance(prev_pt['lat'], prev_pt['lon'], 
                                 curr_pt['lat'], curr_pt['lon'])
        
        # Keep track of total distance
        total_distance += dist
        
        # Only calculate speed if both points have time
        if prev_pt.get('time') and curr_pt.get('time'):
            try:
                # Calculate time difference in seconds
                time_diff = (curr_pt['time'] - prev_pt['time']).total_seconds()
                ...
                if time_diff > 0:
                    ...
                    # Convert to km/h
                    curr_pt['speed'] = speed * 3.6  # m/s to km/h
                    points_with_speed.append(i)
                else:
                    curr_pt['speed'] = 0
            except Exception:
                curr_pt['speed'] = None
    ...
    # Second pass: Estimate speeds for points without time data
    # Calculate average speed if we have enough data
    avg_speed_kmh = 0
    if total_time > 0:
        avg_speed_ms = total_distance / total_time
        avg_speed_kmh = avg_speed_ms * 3.6  # m/s to km/h
    
    # Use average speed or neighbors to estimate missing speeds
    for i in range(1, len(points)):
        if points[i].get('speed') is None:
            # Try to find speed from nearby points
            nearby_speeds = []
            # Look at 5 points before and after
            search_range = 5
            for j in range(max(0, i-search_range), min(len(points), i+search_range+1)):
                if j != i and points[j].get('speed') is not None:
                    nearby_speeds.append(points[j]['speed'])
            
            if nearby_speeds:
                # Use average of nearby speeds
                points[i]['speed'] = sum(nearby_speeds) / len(nearby_speeds)
            elif avg_speed_kmh > 0:
                # Fall back to overall average speed
                points[i]['speed'] = avg_speed_kmh
            else:
                # Last resort: use a reasonable default speed (30 km/h)
                points[i]['speed'] = None
    
    return points


def refine_points(points, known_time_idxs, known_speed_idxs=None, chunk_size=4):
    """
            ...
            if last_known_i is not None and j < n:
                ...
                if d1 == d0:
                    ...
                    if hasattr(t0, 'timestamp'):
                        ...
                        idx_time[i] = datetime.datetime.utcfromtimestamp(new_ts)
                    else:
    ...
    for i in range(n):
        if points[i].get('speed') is None:
            spd_est = interpolate_speed_idw(points,
                                            ...
                                            known_speed_idxs,
                                            power=2)
            points[i]['speed'] = round(spd_est, 1) if spd_est is not None else None
